      ...
      <div class="row">
        <div class="col-md-6 raill-left">
          ...
          <div class="form-group">
            <p for="sel1" style="font-size: 16px">
              <span class="iconfont rb">&#xe60b;</span>&nbsp;选择节点:
            </p>
            <select class="form-control" id="sel2" v-model="nodeObj.nodeSelected">
              <option
                v-for="(item,index) in nodeObj.nodeList"
                :key="index"
                :value="item.id"
              >{{item.name}}</option>
            </select>
            ...
            <p for="sel1" style="font-size: 16px">
              <span class="iconfont rb">&#xe60b;</span>&nbsp;选择传感器类型:
            </p>
            <select class="form-control" id="sel0" v-model="sensorObj.sensorsSelected">
              <option
                v-for="(item,index) in sensorObj.sensorList"
                :key="index"
                :value="item.id"
              >{{item.type}}</option>
            </select>
            <p for="sel1" style="font-size: 16px">
              <span class="iconfont rg">&#xe6af;</span>&nbsp;选择时隔:
            </p>
            <select class="form-control" id="sel" v-model="timeInterval.intervalSelected">
              <option
                v-for="(item,index) in timeInterval.interval"
                :key="index"
                :value="item.id"
              >{{item.type}}</option>
            </select>
          ...
          <div class="form-inline times">
            <p id="time" style="font-size: 16px">
              <span class="rr iconfont">&#xe6e4;</span>&nbsp;选择时间段
            </p>
            <!-- <input class="form-control" id="time_start" type="date" value="2019-09-24"/> -->
            <el-date-picker id="time_start" v-model="time_start" type="date" placeholder="选择日期"></el-date-picker>
            <span>至</span>
            <!-- <input class="form-control" id="time_end" type="date" value="2019-09-24"/> -->
            <el-date-picker id="time_end" v-model="time_end" type="date" placeholder="选择日期"></el-date-picker>
          </div>
          <button type="button" @click="getdata()" class="btn btn-info sub_btn btn-block">提交</button>
        </div>
        <div class="col-md-6 raill-right">
          <div id="chart_example" style="width:560px;height:500px;"></div>
        </div>
      </div>
    </div>
    <!-- main end -->
...
export default {
  ...
  methods: {
    drawLine(this_ = this) {
      let sensor =
        this_.sensorObj.sensorList[this_.sensorObj.sensorsSelected - 1];
      ...
      if (this_.dataObj.date.length === 0) {
        ...
        var html =
          '<div><sapn style="font-size: 18px;font-weight: bold;">图表数据</sapn><span  style="position: absolute;top: 40%;margin-left: 10%;color:#868686; font-size: 20px;">暂无数据</span></div>';
        document.getElementById("chart_example").innerHTML = html;
        document
          .getElementById("chart_example")
    ...
    getdata() {
      ...
      let args = [
        `nid=${this.nodeObj.nodeSelected}`,
        `sensor_id=${this.sensorObj.sensorsSelected}`,
        `start=${String(this.start).substr(0, 10)}`,
        `end=${String(this.end).substr(0, 10)}`,
        `interval=${this.timeInterval.intervalSelected}`
      ];
      Api.detectData(args.join("&"))
        .then(res => {
          console.log(res, "数据");
